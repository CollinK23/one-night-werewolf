...
async function nextRole(roomId: string, current: string | null) {
  const all = await db.select().from(werewolfPlayers).where(eq(werewolfPlayers.roomId, roomId))
...
async function mutate(body: any) {
  ...
  if (action === 'create') {
    ...
    await db.insert(werewolfRooms).values({ id: roomId, code, hostToken: token, centerRoles: [], deckRoles: [], actionSeconds: 30, activeRole: null, actionStartedAt: null, status: 'lobby', phase: 'lobby', updatedAt: new Date() })
  ...
  if (action === 'join') {
    const current = await db.select().from(werewolfPlayers).where(eq(werewolfPlayers.roomId, room.id))
  ...
  if (action === 'settings') {
    ...
    await db.update(werewolfRooms).set({ actionSeconds: Math.min(120, Math.max(10, Number(body.seconds) || 30)), updatedAt: new Date() }).where(eq(werewolfRooms.id, room.id))
    return NextResponse.json({ ok: true })
  }

  if (action === 'start') {
    if (!player.isHost) return NextResponse.json({ error: 'Only the host can start.' }, { status: 403 })
    const players = await db.select().from(werewolfPlayers).where(eq(werewolfPlayers.roomId, room.id)).orderBy(asc(werewolfPlayers.seat))
    if (players.length < 3) return NextResponse.json({ error: 'You need at least 3 players.' }, { status: 400 })
    const deck = shuffle(rolePool).slice(0, players.length + 3)
  ...
  if (action === 'night') {
    ...
    const result: any = { type: player.startingRole, target: body.target, target2: body.target2, center: body.center }

    if (player.startingRole === 'Werewolf') {
      const wolves = players.filter((p) => p.startingRole === 'Werewolf' && p.id !== player.id).map((p) => `Player ${p.seat}`)
      if (wolves.length > 0) result.peek = [`Fellow werewolves: ${wolves.join(', ')}`]
      else if (body.center !== undefined && body.center !== '') result.peek = [`You are the only Werewolf. Center card: ${centers[Number(body.center)]}`]
      else result.peek = ['You are the only Werewolf. Pick a center card below to peek at it (optional).']
    }
    if (player.startingRole === 'Minion') {
      const wolves = players.filter((p) => p.startingRole === 'Werewolf').map((p) => `Player ${p.seat}`)
      result.peek = [wolves.length ? `Werewolves: ${wolves.join(', ')}` : 'No Werewolves are in play. You are alone.']
    }
    ...
    if (player.startingRole === 'Mason') {
      const otherMasons = players.filter((p) => p.id !== player.id && p.startingRole === 'Mason').map((p) => `Player ${p.seat}`)
      result.peek = [otherMasons.length ? `Fellow Mason: ${otherMasons.join(', ')}` : 'No other Mason at the table — the second Mason card is in the center.']
    }
    if (player.startingRole === 'Insomniac') {
      result.peek = [`Your final card is: ${player.role}`]
    }
    if (player.startingRole === 'Doppelgänger') {
      if (!target) return NextResponse.json({ error: 'Choose a player to copy.' }, { status: 400 })
      result.peek = [`You copied Player ${target.seat}: ${target.role}`]
      result.copiedRole = target.role
      await db.update(werewolfPlayers).set({ role: target.role }).where(eq(werewolfPlayers.id, player.id))
    }
  ...
  if (action === 'vote') {
    ...
    const all = await db.select().from(werewolfPlayers).where(eq(werewolfPlayers.roomId, room.id))
